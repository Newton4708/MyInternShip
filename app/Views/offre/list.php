...
<body>
    ...
    <?php if (in_array($_SESSION['user']['statut'], ['administrateur', 'pilote'])): ?>
        <a href="<?= BASE_URL ?>/public/index.php?controller=offre&action=viewcreate">Créer une offre</a>
    <?php endif; ?>

    <form action="<?= BASE_URL ?>/public/index.php?controller=offre&action=list" method="GET" style="margin-top:20px;">
    ...
    <table border="1" cellpadding="5" cellspacing="0" style="margin-top:20px;">
        ...
        <tr>
            ...
            <td>
                <a href="<?= BASE_URL ?>/public/index.php?controller=offre&action=view&id=<?= $offre['id'] ?>">Voir</a>
                <?php if (in_array($_SESSION['user']['statut'], ['administrateur', 'pilote'])): ?>
                    | <a href="<?= BASE_URL ?>/public/index.php?controller=offre&action=edit&id=<?= $offre['id'] ?>">Modifier</a>
                    | <a href="<?= BASE_URL ?>/public/index.php?controller=offre&action=delete&id=<?= $offre['id'] ?>" onclick="return confirm('Confirmez la suppression de cette offre ?');">Supprimer</a>
                <?php endif; ?>
            </td>
        </tr>
        <?php endforeach; ?>
    ...
    <br>
    <a href="<?= BASE_URL ?>/public/">Retour au tableau de bord</a>
</body>
</html>
